...
def f1_score(y_true, y_pred, average='micro', suffix=False):
    ...
    pred_entities = get_pred_entities(y_pred, suffix)
...
def get_pred_entities(y_pred, suffix):
    """当crf_decode_topk>1时，将y_pred拆分"""
    topk = len(y_pred[0][0])
    if not topk > 1:
        ...
        for i in range(topk):
            ...
            for data in y_pred:
                _y_pred = []
                for step in data:
                    _y_pred.append(step[i])
                batch_pred.append(_y_pred)
            split_y_pred.append(batch_pred)
        pred_entities = map(lambda _: set(get_entities(_, suffix)), split_y_pred)
        pred_entities = reduce(lambda x, y: x | y, pred_entities)
...
def precision_score(y_true, y_pred, average='micro', suffix=False):
    ...
    Args:
        y_true : 2d array. Ground truth (correct) target values.
        y_pred : 3d array. Estimated targets as returned by a tagger. with topk result [B, S, topk]
    Returns:
        score : float.
    Example:
