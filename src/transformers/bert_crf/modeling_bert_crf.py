...
from .configuration_bert_crf import BertCrfConfig as BertConfig
...
class BertCrfForTokenClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if config.crf:
            logging.info('模型初始化: 使用crf...')
            self.crf = CRF(config.num_labels, batch_first=True, init_transitions=config.init_transitions)
    ...
    Examples::
        ...
        if labels is not None:
            if hasattr(self, 'crf'):
                # attention_mask:[B, S] / logits:[B, S, T]  / labels:[B, S]
                loss = -self.crf(logits, labels, attention_mask.type(torch.uint8), reduction='mean')
            else:
                loss_fct = CrossEntropyLoss()
