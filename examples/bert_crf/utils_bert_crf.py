...
def transform(e):
    return InputExample(guid=e["index"], words=e["words"], labels=e["labels"])
...
def get_labels(path):
    ...
    try:
        ...
        logging.info('默认采用BIO形式...')
        tags = []
        for l in labels:
            tags.append('B-{}'.format(l))
        tags.append('O')
        for l in labels:
            tags.append('I-{}'.format(l))
        logging.info('原始标签: {}\t训练标签: {}'.format(labels, tags))
